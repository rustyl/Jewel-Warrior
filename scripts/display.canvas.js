...
//console.log("in renderAnimations", anims.n);
        for (i=0;i<n;i++) {
            anim = anims[i];
            if (anim.fncs.before) {
...
//console.log("in display.canvas.js initalize", firstRun);
    ...
    // Listing 9-6 modified by 9-9 then 9-22
    function cycle() {
        var time = Date.now();
        if (!paused) {
...
// adding scale and rotation Listing 9-13
...
//console.log("in refill");
    ...
    function redraw(newJewels, callback) {
        ...
        callback();
    // Listing 8-31
// after Listing 9-7        renderCursor();
    }

    // Listing 8-31 modified with Listing 9-7
    function renderCursor(time){
        ...
        if(cursor.selected) {
            ...
            drawJewel(jewels[x][y], x, y, 1.2, 1.2);
            ctx.restore();
        }
        ctx.save();
...
// now adding the animation Listing 9-11
